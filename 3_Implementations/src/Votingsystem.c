...
void deleteIllegalVote(char userID[15])
{
    ...
    sprintf(filename,"voter%d.txt",voterArray[studentVotes[location-1]-49].cid);
    voterArray[studentVotes[location-1]-49].votes--;
...
int getWinner(){
    ...
    for(int i = 0;i < numberOfVoters; i++){
        if(voterArray[i].votes > maxV) {
            winnerCid = voterArray[i].cid;
            maxV = voterArray[i].votes;      
        }
        else if(voterArray[i].votes == maxV) {
            return -1;
        }
    }
    return winnerCid;
...
void loadElectionInfoFromFile()
{
    ...
    for(int i=1;i<=numberOfVoters;i++)
    {
        ...
        fscanf(f2,"%d",&voterArray[i-1].votes);
        fscanf(f2,"%s",voterArray[i-1].cname);
        while(!feof(f2)){
...
{
    char filename[20];
    sprintf(filename,"voter%d.txt",voteInput-48);
    FILE *fp = fopen(filename,"r+");
    int location=extractRollNo(userID);
    studentVotes[location-1]=voteInput;
    voterArray[voteInput-49].votes++;
    fseek(fp, 0, SEEK_SET);
    fprintf(fp,"%d\n",voterArray[voteInput-49].votes);
    fseek(fp, 0, SEEK_END);
    fprintf(fp,"\n%d",location);
...
void studentPanel()
{
    ...
    while(1)
	{
		...
		for (int i = 0; i < numberOfVoters; i++)
        {
            printf("\n  %d. %s",i+1,voterArray[i].cname);
        }
        printf("\n\n  Your Vote(Enter Number):");
        voteInput=getch();
        printf("*");
        if(voteInput-48 < 1 || voteInput-48 > numberOfVoters)
        {
            printf("\nInvalid Vote\nTry Again...");
            getch();
            continue;
